...
let LoginUserUseCase = class LoginUserUseCase {
    ...
    async execute(input) {
        ...
        try {
            await this.refreshTokenRepository.enforceMaxActiveTokensForUser(user.id, 5);
            const accessToken = this.jwtProvider.generateAccessToken(user.id);
            const { token: rawRefreshToken, expiresAt } = this.jwtProvider.generateRefreshToken(user.id);
